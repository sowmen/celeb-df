...
def get_original_video_paths():
    video_paths = [os.path.join('face_data\\Celeb-real',x) for x in os.listdir('face_data\\Celeb-real')]
    video_paths.extend([os.path.join('face_data\\YouTube-real',x) for x in os.listdir('face_data\\YouTube-real')])
    return video_paths


ROOT_DIR = "face_data"
def main():
    originals = get_original_video_paths()
    print(len(originals))
    _ = []
